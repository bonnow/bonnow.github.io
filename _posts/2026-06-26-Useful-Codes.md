...
```javascript
// (instance address) + nav_to.do?uri= + (table) + .do?sys_id= + (sys_id)
// If the URL above doesn't work, use the one below
// (instance address) + /now/nav/ui/classic/params/target/ + (table) + .do%3Fsys_id%3D + (sys_id)
https://emartdev.service-now.com/nav_to.do?uri=sc_req_item.do?sys_id=${sys_id}

// When you want to set the URL dynamically depending on the instance address
var comm = '@' + gr.request.requested_for.getDisplayValue() + " | ";
if (gs.getProperty('glide.servlet.uri').indexOf('emartdev') == '-1') {
    comm += '[code]<a href="https://eroom.emart.com/workflow/page/form?apmid=' + jsn.result.apm.apmId + '&appid=' + jsn.result.apm.applicationId + '&pop=1" target="blank">' + gr.number + ' Electronic Approval Link </a>[/code]';
} else {
    comm += '[code]<a href="https://eroomdev.emart.com/workflow/page/form?apmid=' + jsn.result.apm.apmId + '&appid=' + jsn.result.apm.applicationId + '&pop=1" target="blank">' + gr.number + ' Electronic Approval Link </a>[/code]';
}
```


### Get the current View value in a UI Action
...
### How to change preference values on the Client Side
...
### Update Set에 record 캡쳐하는 script
...
### 단어로 되어있는 비밀번호 자동 생성 코드 (script include에 저장해두고 실행)
...
### Catalogue Item Variable 간의 종속성 만들기

```javascript
// Department를 고를떄 company 값을 참조해서 list를 제한해야하는 경우
javascript: (function() { return 'cost_centerISNOTEMPTY^company=' + current.variables.ref_company})();
```


### URL에서 dynamic filtering 해서 데이터 제어하기
...
### Catalogue Item Variable에서 현재 날짜를 default value로 설정하는 코드

```javascript
javascript: new Date();
```

### Report에서 날짜 필터 범위 설정 코드
